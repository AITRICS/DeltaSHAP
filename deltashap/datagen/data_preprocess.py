...
# Save processed data
samples = [
    (all_data[i].T, y[i], nan_map[i, :], 1 - masks[i].T)  # Transpose F x T to T x F
...
# patient_vital_preprocessed_all_carryforward_mask_reversed.pkl 파일을 로드합니다.
with open("../data/patient_vital_preprocessed_mask_reversed.pkl", "rb") as f:
...
# 작은 데이터셋을 저장합니다.
with open("../data/patient_vital_preprocessed_mask_reversed_debug.pkl", "wb") as f:
    pickle.dump(small_samples, f)

print("Small datasets created and saved successfully.")
# %%
