...
class IndividualFeatureGenerator(torch.nn.Module):
    ...
    def forward(self, x, past, deterministic=False):
        ...
        Args:
            x:
                The current tensor shape = (num_samples, time_forward, num_features). If
                time_forward is less than predictionsize, padding will be applied. Only used in
                conditional generators.
            past:
                The past tensor. Shape = (num_samples, time_past, num_features)
            deterministic:
                See return.
        ...
        """
        # Permute from (batch_size, time_past, num_features) to (time_past, batch_size, num_features)
        past = past.permute(1, 0, 2)
        _, encoding = self.rnn(past.to(self.device))
        if self.conditional:
            if len(x.shape) == 2:
                x = x.unsqueeze(1)  # Add time dimension if missing
            # Extract all features except the target feature
            x_features = torch.cat(
                (x[:, :, :self.feature_index], x[:, :, self.feature_index+1:]), 2
            ).to(self.device)
            if x.shape[1] != self.prediction_size:
                # not enough current observations
                num_to_pad: int = self.prediction_size - x.shape[1]
                x_features = torch.nn.functional.pad(x_features, [0, 0, 0, num_to_pad])
            x_features = x_features.reshape(x_features.shape[0], -1)
            x = torch.cat((encoding.view(encoding.size(1), -1), x_features), 1)
        else:
    ...
    def forward_all(self, x, deterministic=False):
        ...
        Args:
            x:
                The whole time series. Shape = (num_samples, num_times, num_features)
            deterministic:
                See return.
        ...
        """
        # Permute from (batch_size, num_times, num_features) to (num_times, batch_size, num_features)
        x = x.permute(1, 0, 2)  # (t, bs, f)
        ...
        if self.conditional:
            ...
            x_features = torch.cat(
                (x[:, :, :self.feature_index], x[:, :, self.feature_index+1:]), 2
            ).to(self.device)  # (t, bs, f-1)
            x_features = torch.nn.functional.pad(
                x_features, [0, 0, 0, 0, 0, self.prediction_size - 1]
            )  # (t + p - 1, bs, f - 1)
            x_features = x_features.unfold(0, self.prediction_size, 1)  # (t, bs, f - 1, p)
            x_features = x_features.reshape(x_features.shape[0], x_features.shape[1], -1)  # (t, bs, (f-1)*p)
            x = torch.cat([all_encoding, x_features], 2)  # (t, bs, (h + (f-1)*p)
        else:
            x = all_encoding
    ...
    def run_one_epoch(
        ...
        for i, batch in enumerate(dataloader):
            signals = batch[0]  # (batch_size, time_steps, features)
            label = signals[:, :, self.feature_index].transpose(0, 1)  # (time_steps, batch_size)
            if run_train:
                optimizer.zero_grad()
...
class FeatureGenerator(BaseFeatureGenerator):
    ...
    def forward(self, x, past, deterministic=False):
        ...
        Returns:
            If deterministic, the output will be a tuple of mu and std describing the normal
            distribution. Shape = (num_samples, prediction_size, num_features)
            Otherwise, the output will be sampled from this distribution, together with mu.
            Shape = (num_samples, prediction_size, num_features)
        ...
        firsts = torch.stack(firsts, dim=2)  # (bs, window, feat)
        seconds = torch.stack(seconds, dim=2)  # (bs, window, feat)
    ...
    def forward_all(self, x, deterministic=False):
        ...
        Returns:
            If deterministic, the output will be a tuple of mu and std describing the normal
            distribution. Shape = (num_time, num_samples, prediction_size, num_features)
            Otherwise, the output will be sampled from this distribution, together with mu.
            Shape = (num_time, num_samples, prediction_size, num_features)
        """
        firsts, seconds = [], []
        for model in self.models:
            ...
            firsts.append(first)
            seconds.append(second)
        firsts = torch.stack(firsts, dim=3)  # (t, bs, window, f)
        seconds = torch.stack(seconds, dim=3)  # (t, bs, window, f)
        return firsts, seconds
    ...
    def load_generator(self):
        ...
        """
        for generator_model in self.models:
            print(f"Loading generator from {generator_model._get_model_file_name()}")
