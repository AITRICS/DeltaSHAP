...
class JointFeatureGenerator(BaseFeatureGenerator):
    ...
    def likelihood_distribution(self, mu, std, cov_noise_level=1e-4):
        ...
        while True:
            ...
            if valid.all():
                ...
                # Fix: Create a properly sized noise tensor for the specific indices
                noise_to_add = self.cov_noise.repeat(len(error_index), 1, 1) * cov_noise_level
                covariance[error_index, :, :] = covariance[error_index, :, :] + noise_to_add
                ...
                if count_loop > 20:
                    # Fix: Create a properly sized identity tensor for the specific indices
                    identity_replacement = self.cov_noise.repeat(len(error_index), 1, 1)
                    covariance[error_index, :, :] = identity_replacement
    ...
    def get_z_mu_std(self, past):
        ...
        Args:
            past:
                The past time series. (batch_size, num_times, num_features)
    ...
    def forward_conditional_multisample_from_z_mu_std(
        ...
        Args:
            past:
                The past time series. Shape = (batch_size, num_times, num_features)
            current:
                The current time series for conditioned on.
                Shape (batch_size, time_forward, num_features)
            sig_inds:
        ...
        Returns:
            ...
            Shape = (num_samples, batch_size, time_forward, num_features)
        ...
        intermediate = torch.linalg.solve(cov_2_2, cov_1_2.transpose(1, 2)).transpose(
            1, 2
        )
        ...
        if len(error_indexes) > 0:
            ...
            for error_index in error_indexes:
                ...
                # full_sample.shape = (n_samples, batch_size, time_forward, num_features)
                full_sample[
                    original_sample_index, original_batch_index, :, old_sig_inds
                ] = past[original_batch_index, -1:, old_sig_inds]
    ...
    def _run_one_epoch(
        ...
        for i, batch in enumerate(dataloader):
            signals = batch[0]  # signals shape: (batch_size, num_times, num_features)
            if num == 1:
                timepoints = [signals.shape[1] - self.prediction_size]
            else:
                timepoints = [
                    int(tt)
                    for tt in np.logspace(
                        1.0, np.log10(signals.shape[1] - self.prediction_size), num=num
                    )
                ]
            ...
            for t in timepoints:
                if run_train:
                    optimizer.zero_grad()
                # Extract label from signals with shape (batch_size, time_forward, num_features)
                # and reshape to (batch_size, time_forward * num_features)
                label = (
    ...
    def load_generator(self):
        ...
        """
        print(f"Loading generator from {self._get_model_file_name()}")
